...
export class PerifericosService {
  ...
  //get buscar
  async buscar(perifericoConsultaDto) {
    const perifericosModel =
      await this.perifericosRepository.query(perifericoConsultaDto);
    return perifericosModel.map(
      (perifericoModel) => new PerifericosResponseDto(perifericoModel),
    );
  }
