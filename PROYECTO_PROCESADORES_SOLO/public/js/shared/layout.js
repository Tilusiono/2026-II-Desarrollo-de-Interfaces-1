...
async function cargarComponentes() {
  ...
  await Promise.all(
    [...contenedores].map(async (contenedor) => {
      const modelo = contenedor.dataset.component;
      const ruta = COMPONENTES[modelo];
      if (!ruta) return;

      const respuestaHttp = await fetch(ruta);
      contenedor.innerHTML = await respuestaHttp.text();
      if (modelo === "header") completarHeader(contenedor);
    }),
  );
...
function completarHeader(header) {
  ...
  const eyebrow = header.querySelector("[data-header-eyebrow]");
  if (eyebrow) eyebrow.textContent = configuracionHeader.eyebrow ?? "Administración técnica";
